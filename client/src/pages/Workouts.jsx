...
export default function Workouts() {
  ...
  useEffect(() => {
    if (user) {
      axios
        .get(`/api/savedWorkouts/${user._id}`)
        .then(({ data }) => {
    ...
    <div className="workouts-container">
      ...
      <div className="section">
        ...
        <div className="workouts-grid">
          {savedPremadeWorkouts.length ? (
            savedPremadeWorkouts.map((workout) => (
              <div
                key={workout._id}
                className="workout-card"
                onClick={() => navigate(`/premade-workouts/${workout._id}`)}
          ...
          {customWorkouts.length ? (
            customWorkouts.map((workout, idx) => (
              <div
                ...
                className="workout-card"
                onClick={() =>
                  navigate(`/custom-workouts/${user._id}/${workout._id}`)
                }
                style={{ cursor: "pointer" }}
              >
                <h4>{workout.name}</h4>
                {workout.description && (
                  <p style={{ marginTop: "0.5rem" }}>{workout.description}</p>
                )}
              </div>
            ))
          ) : (
      ...
      {showPremade && (
        <div className="modal">
          <div className="modal-content">
            ...
            <button className="close-btn" onClick={() => setShowPremade(false)}>
              X
            </button>
            <div className="workouts-grid">
